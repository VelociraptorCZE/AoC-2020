const bagRules = `drab plum bags contain 5 clear turquoise bags, 5 striped aqua bags, 4 dotted gold bags, 4 plaid chartreuse bags.
...
light fuchsia bags contain 5 light violet bags, 2 striped aqua bags.`.split("\n");

const bags = {};

bagRules.forEach(bagRule => {
	let [bagName, bagsInside] = bagRule.split("contain");
	bagName = bagName.replace("bags", "").trim();
	
	const bagsInsideList = {};

	bagsInside.split(",").forEach(bag => {
		const name = bag.replace(/\d|\.|bags|bag/g, "").trim();
		
		if (name === "no other") {
			return;
		}
		
		bagsInsideList[name] = +bag.match(/\d/g)[0];
	});
	
	bags[bagName] = bagsInsideList;
});

const getRecursiveListOfAllBagsInside = (bagColor, bagList = []) => {
	const bagsInside = Object.keys(bags[bagColor]);
	
	for (const bagColor of bagsInside) {
		bagList.push(bagColor, ...Object.keys(bags[bagColor]));
		
		if (bagColor !== "shiny gold") {
			getRecursiveListOfAllBagsInside(bagColor, bagList);
		}
	}
...
const firstPart = () => Object
	.keys(bags)
	.reduce((bagCount, bagColor) => bagCount + getRecursiveListOfAllBagsInside(bagColor).includes("shiny gold"), 0);

console.log("Results", firstPart());
