...
const firstPart = () => (
  answers
    .map(answerSheet => [...new Set(answerSheet.replace(/\n/g, "").split(""))].join(""))
    .reduce((answerCount, answerSheet) => answerCount + answerSheet.length, 0)
)

const secondPart = () => answers.reduce((answerCount, answerSheet) => {
  const answersByLines = answerSheet.split("\n");

  if (answersByLines.length === 1) {
    return answerCount + answerSheet.length;
  }

  const answersByQuestions = {};

  for (let i = 0; i < answersByLines.length; ++i) {
    const answerLine = answersByLines[i];

    for (let j = 0; j < answerLine.length; ++j) {
      const singleAnswer = answerLine[j];
      let isInvalid = false;

      for (let k = 0; k < answersByLines.length; ++k) {
        if (!answersByLines[k].includes(singleAnswer)) {
          isInvalid = true;
        }
      }
      ...
      if (!answersByQuestions[singleAnswer]) {
        answersByQuestions[singleAnswer] = 1;
      } else {
        ++answersByQuestions[singleAnswer];
      }
    }
  }

  return answerCount + Object.values(answersByQuestions).reduce((a, ac) => ac > 1 ? a + 1 : a, 0);
}, 0);
