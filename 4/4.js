...
const isPassportValid = passport => {
	const passportFields = passport.match(/byr|eyr|pid|hgt|iyr|ecl|hcl|cid/g);

	return passportFields.length === 8 || passportFields.length === 7 && !passportFields.includes("cid");
};

const firstPart = () => credentials.filter(passport => isPassportValid(passport)).length;

const secondPart = () => (
	credentials.filter(passport => {
		...
		const validationRules = {
			...
			hcl: value => value.match(/^#[a-fA-F0-9]{6}$/),
			pid: value => value.match(/^\d{9}$/),
			cid: () => true
		};

		const passportCredentials = passport.split(/\n| /).map(keyValuePair => keyValuePair.trim().split(":"));
		
		return passportCredentials.every(([key, value]) => validationRules[key](value));
	}).length
);
