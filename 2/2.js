const passwords = `1-13 r: gqdrspndrpsrjfjx
...
6-7 c: cccccdqcc
2-6 x: vjkxbrfwnj
16-18 s: kssssssswssssssssssb`.split("\n");

const parsePasswordTemplate = template => template.replace(":", "").split(" ");

const firstPart = () => (
    passwords.filter(passwordTemplate => {
        const [allowedRepetitions, character, password] = parsePasswordTemplate(passwordTemplate);
        const numberOfMatches = +password.match(new RegExp(character, "g"))?.length;
        const [minRepetitions, maxRepetitions] = allowedRepetitions.split("-").map(Number);

        return numberOfMatches >= minRepetitions && numberOfMatches <= maxRepetitions;
    }).length
);

const secondPart = () => (
    passwords.filter(passwordTemplate => {
        const [allowedIndexes, character, password] = parsePasswordTemplate(passwordTemplate);
        const [i1, i2] = allowedIndexes.split("-");

        return (password[i1 - 1] + password[i2 - 1]).match(new RegExp(character, "g"))?.length === 1;
    }).length
);
